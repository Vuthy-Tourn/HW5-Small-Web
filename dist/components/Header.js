...
function Header() {
    ...
    `;
      ...
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16 lg:h-20">
                ...
                <li>
                  <a href="${link.href}" 
                     class="relative py-2 px-1 font-medium transition-all duration-300 group
                             hover:text-blue-600
                            dark:text-gray-300 dark:hover:text-blue-400 ${activeNav ? "text-black" : "text-gray-500"}">
                    ${link.name}
          ...
          <div class="flex items-center space-x-4">
            ...
            <button id="dark-mode-toggle" class="p-2 rounded-lg transition-all duration-200
                       hover:bg-gray-100
                      dark:text-gray-300 dark:hover:bg-gray-700 ${activeNav ? "text-black" : "text-gray-500"}">
              ${darkMode
            ? `
            ...
            <button id="mobile-toggle" class="lg:hidden p-2 rounded-lg transition-all duration-200
              ...
              <div class="w-6 h-6 flex flex-col justify-center space-y-1">
                <span class="block h-0.5 bg-current transition-all duration-300 ${toggleBtn ? "rotate-45 translate-y-1.5" : ""}"></span>
                <span class="block h-0.5 bg-current transition-all duration-300 ${toggleBtn ? "opacity-0" : ""}"></span>
                <span class="block h-0.5 bg-current transition-all duration-300 ${toggleBtn ? "-rotate-45 -translate-y-1.5" : ""}"></span>
              </div>
            </button>
          </div>
        ...
        <!-- Mobile Menu -->
        <div class="lg:hidden overflow-hidden transition-all duration-300 ${toggleBtn ? "max-h-screen opacity-100" : "max-h-0 opacity-0"}">
    ...
    const setupEventListeners = () => {
        ...
        const mobileDarkModeToggle = navbar.querySelector("#mobile-dark-mode-toggle");
    ...
    updateNavbar();
    navbar.updateCartAmount = (newAmount) => {
        amount = newAmount;
        updateNavbar();
    };
    navbar.updateCheckoutPath = (newPath) => {
