...
exports.default = createProductCard;
function createProductCard(product) {
    ...
    <div class="product-card flex flex-col justify-between dark:bg-gray-800/50 rounded-2xl overflow-hidden shadow-[0_4px_24px_rgba(0,0,0,0.05)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.2)]
      ...
      <a href="/product/${product.id}" data-link class="flex flex-col justify-between">
        ...
        <div class="px-6 py-4">
          <div class="mb-2">
            <span class="text-xs text-[#F35F25] font-medium uppercase tracking-wide">${product.brand || ""}</span>
          </div>
          ...
          <div class="flex items-center justify-between py-2">
            <div class="flex items-center gap-2 justify-center">
              ${product.discountPercentage > 0
        ? `
                <span class="text-2xl font-bold text-[#F35F25]">$${discountedPrice.toFixed(0)}</span>
        ...
        : `
          ...
          <button class="w-full py-3 bg-[#1649A1] text-white font-medium rounded-lg hover:bg-[#162ba1] transition-colors flex items-center justify-center gap-2 relative z-10" ${product.stock === 0 ? "disabled" : ""}>
